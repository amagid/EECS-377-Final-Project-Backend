...
```
mysql -u root -p alexa_lampi < path/to/dbdump.sql

mysql -u root -p alexa_lampi_testing < path/to/dbdump-testing.sql
```
...
section, the username and password can be found in the *db-init.sql* file; use the information for the **alexa_lampi_server** user. **Please note that some external services, including the Google 
